    ...
    public class SensorTransform : MonoBehaviour, IPointerClickHandler
    {
        ...
        public void UpdateRotation(ImuDataFrame vFrame)
        {
            if (!UseBodyOrientation)
            {
                ...
                Vector3 vMagVec   = new Vector3(vFrame.Mag_x, vFrame.Mag_y, vFrame.Mag_z); ///extraction des donnees de champ magnetique directement des capteurs
                Rotation.CurId = SensorPos;
                Rotation.UpdateAccel(vAccelVec);
                Rotation.UpdateGyro(vGyroVec);
                Rotation.UpdateMag(vMagVec); ///Mise 'a jour du vecteur champ magnetique provenant directement des capteurs
