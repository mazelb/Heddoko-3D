        ...
        private function GetCurrentFileInfo () 
        {
            ...
            if (infoExists) 
            {
                try 
                {
                    ...
                    //windows: check if the current directory is a junction point. If so ignore it. 
                    var vSkippableIndex:int  = 0;
                    if (filePath == "C:\\\\") {
                        ...
                        var vSecondIndex = 0;
                        //remove Documents and Settings from dirInfo;
                        var vNewDirInfo :
                        DirectoryInfo[] = new DirectoryInfo[dirInfo.length - 1];
                        for (var l = 0; l < dirInfo.length ; l++) {
                            if (l == vSkippableIndex) {
                                continue;
                            }
                            vNewDirInfo[vSecondIndex] = dirInfo[l];
                            vSecondIndex++;
                        }
                        dirInfo = vNewDirInfo;
                    }
            ...
            if (!limitToInitialFolder && dirInfo.Length > 0) 
            {
                for (var i = 0; i < dirInfo.Length; i++) {
                    ...
                    if (doFolderFunction) 
                    {
                        ...
                        if (!folderFunction(filesInFolder)) 
                        {
                            continue;
                        }
                    }
